...
export type SpotifyPlaybackState = 'idle' | 'starting' | 'playing' | 'handoff' | 'error'

// Auto mode behaves like a film score: a scene needs to change materially and
// settle before another track is allowed to replace the current one.
...
export const useSpotifyStore = create<SpotifyState>((set, get) => ({
  ...
  requestSuggestion: async (classification) => {
    ...
    try {
      const suggestion = await window.fuzzy.spotify.suggestForMood(classification, { excludeUris })
  ...
  playSuggestion: async (suggestionToPlay) => {
    ...
    try {
      ...
      if (result.started) {
        ...
        set({
          playbackState: 'handoff',
          playbackMessage: result.message,
          undoSnapshot: null
        })
      } else {
  ...
  soundtrackPassage: async (classification) => {
    const suggestion = await get().requestSuggestion(classification)
    if (!suggestion) return null
